...
$pokebot_logpos   = nil
$pokebot_logview1 = "/bin/grep 'Captured\\|awarded\\|Total\\|appeared\\|Releasing\\|Exchanged\\|Discarded\\|renamed\\|reward\\|incubates' /opt/PokemonGo-Bot/log/<utente>.log | /usr/bin/tail -c 4090 | /usr/bin/tail -n +2 | /usr/bin/awk '{print $2" "substr($0,index($0,$5))}' | /bin/sed 's/SpinFort\\] \\[INFO\\] //' | /usr/bin/awk '{print $1" "substr($0,index($0,$3))}' | /usr/bin/awk '{print substr($1,0,9)" "substr($0,index($0,$2))}'"
$pokebot_logview2 = "/bin/grep 'Captured\\|awarded\\|Total\\|appeared\\|Releasing\\|Exchanged\\|Discarded\\|renamed\\|reward\\|incubates' /opt/PokemonGo-Bot/log/<utente>.log | /usr/bin/awk '$0 > \"#{$pokebot_logpos}\"' | /usr/bin/tail -c 4090 | /usr/bin/tail -n +2 | /usr/bin/awk '{print $2" "substr($0,index($0,$5))}' | /bin/sed 's/SpinFort\\] \\[INFO\\] //' | /usr/bin/awk '{print $1" "substr($0,index($0,$3))}' | /usr/bin/awk '{print substr($1,0,9)" "substr($0,index($0,$2))}'"
$pokebot_loglast  = "/bin/grep 'Captured\\|awarded\\|Total\\|appeared\\|Releasing\\|Exchanged\\|Discarded\\|renamed\\|reward\\|incubates' /opt/PokemonGo-Bot/log/<utente>.log | /usr/bin/tail -1 | /usr/bin/awk '{print $1\" \"$2}'"
